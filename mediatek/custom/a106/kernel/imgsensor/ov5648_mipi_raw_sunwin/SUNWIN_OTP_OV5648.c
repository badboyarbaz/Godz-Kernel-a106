...
#include "ov5648mipi_Camera_Sensor_para_sunwin.h"
#include "ov5648mipi_CameraCustomized_sunwin.h"
...
#define TRACE(fmt, arg...) printk("[OV5648MIPIRaw_OTP_SUNWIN]%s: " fmt "\n", __FUNCTION__ ,##arg)//LINE <> <DATE20130923> <ov5648 OTP log> wupingzhou

extern kal_uint16 OV5648MIPI_read_cmos_sensor_sunwin(kal_uint32 addr);
extern kal_uint16 OV5648MIPI_write_cmos_sensor_sunwin(kal_uint32 addr, kal_uint32 para);
...
#define OV5648_write_cmos_sensor(addr,para) OV5648MIPI_write_cmos_sensor_sunwin(addr,para)
#define OV5648_read_cmos_sensor(addr) OV5648MIPI_read_cmos_sensor_sunwin(addr)
...
//	else if ((!(flag&0x80)) && ((flag&0x7f) == OTP_MID) && lens_id ==LENS_ID)
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
bool otp_update_wb_sunwin(unsigned short golden_rg, unsigned short golden_bg) 
{
	...
	if (otp_read_wb_group(-1) != -1)
	{
		if (otp_apply_wb(golden_rg, golden_bg) == 1)
		{
			...
			// mdelay(1);
			//TRACE("the current sunwin module's R/G value is  0x%x", RG_MSB );
			 //mdelay(1);
			//TRACE("the current sunwin module's B/G value is  0x%x", BG_MSB );
			return 1;
		}
	}
...
signed char check_sunwin_MID(signed char index)
	{


/*
		if(otp_read_wb_group(index)!=-1)		//it is sunwin module
		
			{

				TRACE("it is sunwin module\n");
...
*/
		...
		for (index=0; index<3; index++)
		{
			if (otp_check_wb_group(index) == 2)
			{
				...
				TRACE("check_sunwin_MID sucessed %d", index);		
