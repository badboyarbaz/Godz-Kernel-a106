...
#define TRACE(fmt, arg...) printk("[OV5648MIPIRaw_OTP_truly]%s: " fmt "\n", __FUNCTION__ ,##arg)//LINE <> <DATE20130923> <ov5648 OTP log> wupingzhou

extern kal_uint16 OV5648MIPI_read_cmos_sensor_truly(kal_uint32 addr);
extern kal_uint16 OV5648MIPI_write_cmos_sensor_truly(kal_uint32 addr, kal_uint32 para);
...
#define OV5648_write_cmos_sensor(addr,para) OV5648MIPI_write_cmos_sensor_truly(addr,para)
#define OV5648_read_cmos_sensor(addr) OV5648MIPI_read_cmos_sensor_truly(addr)
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
bool otp_update_wb_truly(unsigned short golden_rg, unsigned short golden_bg) 
{
...
signed char check_truly_MID(signed char index)
	{
...
*/
		...
		for (index=0; index<3; index++)
		{
			if (otp_check_wb_group(index) == 2)
			{
				...
				TRACE("check_truly_MID sucessed %d", index);		
