...
#include "ov5648mipi_Sensor_darling_zl.h"
#include "ov5648mipi_Camera_Sensor_para_darling_zl.h"
#include "ov5648mipi_CameraCustomized_darling_zl.h"
...
#define TRACE(fmt, arg...) printk("[OV5648MIPIRaw_OTP_DARLING_ZL]%s: " fmt "\n", __FUNCTION__ ,##arg)//LINE <> <DATE20130923> <ov5648 OTP log> wupingzhou

extern kal_uint16 OV5648MIPI_read_cmos_sensor_darling_zl(kal_uint32 addr);
extern kal_uint16 OV5648MIPI_write_cmos_sensor_darling_zl(kal_uint32 addr, kal_uint32 para);
...
#define OV5648_write_cmos_sensor(addr,para) OV5648MIPI_write_cmos_sensor_darling_zl(addr,para)
#define OV5648_read_cmos_sensor(addr) OV5648MIPI_read_cmos_sensor_darling_zl(addr)
...
* Return      :  1, success; 0, fail
*******************************************************************************/	
bool otp_update_wb_darling_zl(unsigned short golden_rg, unsigned short golden_bg) 
{
...
signed char check_darling_zl_MID(signed char index)
	{


/*
		if(otp_read_wb_group(index)!=-1)		//it is darling_zl module
...
*/
		...
		for (index=0; index<3; index++)
		{
			if (otp_check_wb_group(index) == 2)
			{
				...
				TRACE("check_darling_zl_MID sucessed %d", index);		
		...
		TRACE("check_darling_zl_MID failed");
